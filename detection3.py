...
# Registered cars
for img_path in glob.glob(dir + "/*.jpeg"):
...
for img_path in glob.glob(dir + "/*.jpeg"):
    ...
    number_plate = number_plate_detection(img)
    search_plate = "".join(re.split("[^a-zA-Z0-9]*", number_plate)).upper()

    if search_plate != "":
        break   # first valid plate milte hi stop

print("The car number to search is:-", search_plate)

result = binarySearch(array, 0, len(array) - 1, search_plate)
